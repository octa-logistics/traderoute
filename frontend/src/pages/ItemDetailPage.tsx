...
interface ItemDetail {
  ...
  processingLogs: Array<{ id: string; description: string; cost: number; date: string }>;
  costs: {
...
export default function ItemDetailPage() {
  ...
  useEffect(() => {
    apiFetch<ItemDetail | null>(`/items/${id}`, null).then((data) => {
      setItem(data);
      setLoading(false);
    });
  }, [id]);
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link to="/dashboard/items" className="text-sm text-slate-500 hover:text-slate-700">&larr; Items</Link>
          </div>
            ...
            {item.purchaseDeal && (
              <div className="flex justify-between">
                ...
                <dd className="text-sm">
                  <Link to={`/dashboard/purchases/${item.purchaseDeal.id}`} className="text-slate-900 hover:text-slate-700 underline">
