...
export default function PurchaseDetailPage() {
  ...
  const [deal, setDeal] = useState<PurchaseDetail | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    apiFetch<PurchaseDetail | null>(`/purchase-deals/${id}`, null).then((data) => {
      setDeal(data);
      setLoading(false);
    });
  }, [id]);
  ...
  if (!deal) return <p className="text-slate-500">Purchase deal not found.</p>;
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1">
        <Link to="/dashboard/purchases" className="text-sm text-slate-500 hover:text-slate-700">&larr; Purchase Deals</Link>
      </div>
            ...
            {deal.items.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-4 py-3">
                  <Link to={`/dashboard/items/${item.id}`} className="text-sm font-medium text-slate-900 hover:text-slate-700">
                    {item.name}
                  </Link>
